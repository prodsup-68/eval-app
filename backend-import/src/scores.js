...
async function importData() {
  try {
    ...
    const data = JSON.parse(
      await readFile("./src/data/S03_app_scores.json", "utf-8"),
    );
    const collectionName = "scores"; // Replace with your collection's name

    // 4. Clear existing records in the target collection before importing new data
    const records = await pb.collection(collectionName).getFullList();
    for (const record of records) {
      await pb.collection(collectionName).delete(record.id);
    }

    // 5. Process and create records in chunks
